...
class Window:
    ...
    def get_selected_row(self,event):
        ...
        ndex=self.list1.curselection()
        if ndex != ():
            self.selected_tuple=self.list1.get(ndex[0])
    ...
    def search_command(self):
        self.list1.delete(0,END)
        for row in database.search(title=self.title_text.get(), author=self.author_text.get(), year=self.year_text.get(), isbn=self.isbn_text.get()):
            self.list1.insert(END,row)
